...
Dividing a dataset into two subsets: one to train the model (training set) and one to evaluate its performance on unseen data (testing set), also there is Validation Set which is part from training set that you as developer use it to adjust hyper parameters and choose best model then pass this model to the test set avoiding that the best model is driven from the test set itself.  
